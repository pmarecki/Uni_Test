/*
 * Basic operations on `Float2` numbers
 * imitating `double` up to ~50ulp; the file is essentially the "dsmath.h"
 * found all over the net.
 */

#ifndef _DSMATH_PI_H_
#define _DSMATH_PI_H_

//only modification of the original "dsmath.h"
// CUDA-specific modifiers defined to " " if no _CUDA_ declared
#ifndef _CUDA_
#define __device__
#define __host__
#endif
...
__host__ void set(double &a, float2 b) {a = b.x+b.y;}
...
float2 operator*(const float2 a, const float2 b)
{
  ...
  ml  = (((__fmul_rn(u1, v1) - mh) + __fmul_rn(u1, v2))
      + __fmul_rn(u2, v1))      + __fmul_rn(u2, v2);
...
__device__ inline float2 operator/(const float2 a, const float2 b)
{
  ...
  c21 = (((__fmul_rn(a1, b1) - c11) + __fmul_rn(a1, b2))
      + __fmul_rn(a2, b1))       + __fmul_rn(a2, b2);
